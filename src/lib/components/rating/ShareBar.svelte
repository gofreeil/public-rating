...
<div class="flex flex-wrap items-center gap-1.5" role="group" aria-label="שיתוף">
    ...
    {#each NETWORKS as n (n.key)}
        <a
            ...
            class="share-btn flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/5 text-sm transition-colors"
            style="--hover: {n.color}"
        >
    ...
    <button
        ...
        title="העתקת קישור"
        class="share-btn flex h-8 cursor-pointer items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2.5 text-xs font-bold transition-colors {copied
            ? 'text-emerald-300'
            : 'text-gray-300'}"
        style="--hover: #60a5fa"
    >
    ...
    {#if canNativeShare}
        <button
            type="button"
            onclick={nativeShare}
            class="share-btn flex h-8 cursor-pointer items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2.5 text-xs font-bold text-gray-300 transition-colors sm:hidden"
            style="--hover: #a78bfa"
        >
