<script lang="ts">
    ...
    const TONE: Record<string, string> = {
        insufficient: 'border-white/10 bg-slate-800/80 text-gray-400',
...
<div class="flex flex-wrap items-center gap-2 text-xs">
    ...
    <span class="rounded-full border border-white/10 bg-slate-800/80 px-2.5 py-1 text-gray-400">
        👥 {stats.count}
    ...
    {#if stats.count > 0}
        <span
            class="rounded-full border border-white/10 bg-slate-800/80 px-2.5 py-1 text-gray-400"
